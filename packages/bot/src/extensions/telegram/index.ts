...
function log(...a: unknown[]): void { console.log("[telegram]", ...a); }

export interface TelegramExtensionOptions { config: BotConfig; onAbort?: () => void; onNew?: () => void; onCompact?: () => void; }
...
export default function telegramExtension(pi: ExtensionAPI, opts: TelegramExtensionOptions): void {
	...
	if (!token) { console.error("[telegram] no token"); return; }
	...
	const CMDS: Record<string, (c: number, a: string) => Promise<void>> = {
		...
		"/abort": async (c) => { if (busy) { onAbort?.(); await bot.sendMessage(c, "Aborted."); } else await bot.sendMessage(c, "Idle."); },
		"/new": async (c) => { onNew?.(); await bot.sendMessage(c, "Fresh."); },
		"/compact": async (c) => { if (!busy) { onCompact?.(); await bot.sendMessage(c, "Compacting..."); } else await bot.sendMessage(c, "Busy."); },
		"/status": async (c) => { await bot.sendMessage(c, `Status: ${busy ? "busy" : "idle"}`); },
		...
		if (!msg.text) {
			if (msg.photo?.length) {
				try {
					...
					const p = parseDataUrl(du); if (!p) { await bot.sendMessage(msg.chatId, "Img fail"); return; }
					pi.sendUserMessage([{ type: "text" as const, text: msg.caption || "Analyze" }, { type: "image" as const, data: p.data, mimeType: p.mimeType }], busy ? { deliverAs: "followUp" } : undefined);
				} catch { await bot.sendMessage(msg.chatId, "DL fail"); }
			}
			return;
		}
		const t = msg.text.trim(); if (!t) return;
		const cmd = t.split(/\s/)[0]?.toLowerCase();
		if (cmd && cmd in CMDS) { await CMDS[cmd](msg.chatId, t.slice(cmd.length).trim()); return; }
	...
	pi.on("turn_end", async (e) => {
		...
		if (ev.message?.usage) { const t = (ev.message.usage.input ?? 0) + (ev.message.usage.output ?? 0); await appendWorking(`${t.toLocaleString()} tok`); }
	});
	pi.on("agent_end", () => {
	...
	log("starting poll...");
	bot.start().then(() => log("poll OK")).catch((e) => log("poll FAIL:", e.message));
	log("ready");
}
...
function toolSummary(n: string, a: unknown): string {
	...
	switch (n) { case "bash": return typeof o.command === "string" ? trunc(o.command, 60) : n; case "read": case "edit": case "write": return typeof o.path === "string" ? o.path : n; default: return n; }
}
function trunc(s: string, m: number): string { return s.length <= m ? s : `${s.slice(0, m - 3)}...`; }
